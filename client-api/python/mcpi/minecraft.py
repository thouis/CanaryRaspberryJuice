...
class Minecraft:
    ...
    def setBlocks(self, *args):
        ...
        pos_args = [str(v) for v in intFloor(args[:6])]
        block_id = bytearray(args[6] if isinstance(args[6], collections.Iterable) else [args[6]])
        if len(args) > 7:
            data = bytearray(args[7] if isinstance(args[7], collections.Iterable) else [args[7]])
        else:
            data = bytearray([0])
